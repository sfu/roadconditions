...
exports.init = function(config, logger) {
    ...
    pub.on('connect', function() { pubsub.emit('connect', 'publisher'); });
    pub.on('error', function(err) { pubsub.emit('error', 'publisher', err); });
    sub.on('connect', function() {
        pubsub.emit('connect', 'subscriber');
        sub.subscribe(config.channel);
    });
    sub.on('error', function(err) { pubsub.emit('error', 'publisher', err); });
