...
;(function(Spine, $, exports, data) {
  ...
  Condition.extend({
    ...
    hydrate: function(campus) {
      var conditionsMap = JSON.parse(
        document.getElementById('conditions_map').innerHTML
      )
  ...
  var App = Spine.Controller.sub({
    ...
    init: function() {
      ...
      $('#updateconditions').on('submit', function(ev) {
        ev.preventDefault()
      })
      ...
      $('#updateconditions')
        ...
        .validate({
          ...
          invalidHandler: function(form, validator) {
            ...
            msg +=
              'highlighted below. Please correct the problem and try again.'
            msg = $('<div>').html(msg)
            window.scrollTo(0, 1)
            $('#message-container')
              .removeClass()
          ...
          submitHandler: function(form) {
            ...
            for (var i = 0; i < data.announcements.length; i++) {
              data.announcements[i] = data.announcements[i].replace(
                nbspregex,
                ''
              )
            }
            ...
            $.ajax({
              ...
              success: function(data) {
                $('#lastupdated span').text(
                  moment(new Date(data.lastupdated)).format(
                    '[at] h:mm a [on] dddd, MMMM DD, YYYY'
                  )
                )
                var msg = $('<div>').html(
                  '<strong>Success!</strong><br/>The Road and Traffic Report has been updated.'
                )
                $('#message-container')
                  .removeClass()
                  .addClass('success')
              ...
              error: function() {
                var msg = $('<div>').html(
                  '<strong>Oh dear, something has gone awry.</strong><br/>The server reported an error when trying to process the form. Please wait a moment and try again.'
                )
                $('#message-container')
                  .removeClass()
                  .addClass('error')
  ...
  Spine.$,
  window.exports,
  JSON.parse(document.getElementById('conditions_json').innerHTML)
)
